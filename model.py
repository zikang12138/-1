...
class Actionness_Module(nn.Module):
    def __init__(self, len_feature, num_classes):
        ...
        self.f_embed = nn.Sequential(#Feature Embedding 得到XE
            nn.Conv1d(in_channels=self.len_feature, out_channels=2048, kernel_size=3,
                      stride=1, padding=1),
            nn.ReLU()
        )

        self.f_cls = nn.Sequential(#Actionness Modeling 得到CAS
    ...
    def forward(self, x):
        out = x.permute(0, 2, 1)#将x的维度换位 batch*2048*segement 32*2048*750
        out = self.f_embed(out)#得到XE 32*2048*750
        embeddings = out.permute(0, 2, 1)#再换回来32*750*2048
        out = self.dropout(out)
        out = self.f_cls(out)#32*20*750 公式1
        cas = out.permute(0, 2, 1)#32*750*20 
        actionness = cas.sum(dim=2)#32*750 公式2 sum
...
class CoLA(nn.Module):
    def __init__(self, cfg):
        ...
        self.len_feature = cfg.FEATS_DIM#2048
        self.num_classes = cfg.NUM_CLASSES#20
    ...
    def select_topk_embeddings(self, scores, embeddings, k):
        ...
        idx_topk = idx_DESC[:, :k]#batch_size*k
        idx_topk = idx_topk.unsqueeze(2).expand([-1, -1, embeddings.shape[2]])#32*150*2048
        selected_embeddings = torch.gather(embeddings, 1, idx_topk)#因为scores未必是真实值,选择对应最值位置的XE
        return selected_embeddings

    def easy_snippets_mining(self, actionness, embeddings, k_easy):
        select_idx = torch.ones_like(actionness).cuda()#32*750
        select_idx = self.dropout(select_idx)#32*750

        actionness_drop = actionness * select_idx#32*750

        actionness_rev = torch.max(actionness, dim=1, keepdim=True)[0] - actionness#32*1-32*750 通过最大减最小 获得逆序
        actionness_rev_drop = actionness_rev * select_idx

        easy_act = self.select_topk_embeddings(actionness_drop, embeddings, k_easy)#公式7 32*150*2048 k_easy=150
        easy_bkg = self.select_topk_embeddings(actionness_rev_drop, embeddings, k_easy))#公式7 32*150*2048 k_easy=150
    ...
    def forward(self, x):
        ...
        k_easy = num_segments // self.r_easy#r 超参数
